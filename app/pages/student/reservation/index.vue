...
const visibleBookings = computed(() =>
  (bookingsData.value ?? []).slice(0, visibleCount.value)
);
const hasMore = computed(
  () => visibleCount.value < (bookingsData.value?.length ?? 0)
);
  ...
  <div class="container mx-auto p-4 md:p-6">
    <div class="flex flex-col lg:flex-row gap-6 items-start">
      ...
      <aside class="w-full lg:w-80 xl:w-96 shrink-0">
        ...
        <div v-if="!bookingsData || bookingsData.length === 0" class="alert">
          <span>{{ t("student.bookings.no_bookings") }}</span>
        </div>
            ...
            <div class="card-body p-4">
              <div class="flex flex-col gap-2">
                <div class="flex flex-wrap items-center gap-2">
                  <h3 class="font-semibold text-sm">{{ booking.slotTitle }}</h3>
                ...
                <div class="flex flex-col gap-1 text-xs opacity-70">
                  <span class="inline-flex items-center gap-1">
                    <Icon name="heroicons-outline:calendar" class="h-3.5 w-3.5" />
                    {{ formatDate(booking.slotStartTime) }}
                  </span>
                  <span class="inline-flex items-center gap-1">
                    <Icon name="heroicons-outline:clock" class="h-3.5 w-3.5" />
