...
const handleEmailAuth = async () => {
  ...
  try {
    if (isLoginMode.value) {
      ...
      if (error) {
        errorMessage.value = error.message || t("auth.error_generic");
        return;
      }
    } else {
    ...
    errorMessage.value = e.message || t("auth.error_generic");
  } finally {
      ...
      <div class="card shrink-0 w-full max-w-sm shadow-2xl bg-base-100">
        <div class="card-body">
          <div class="flex justify-center mb-4">
            <a
              class="tab transition-colors duration-200 hover:bg-primary hover:text-primary-content"
          ...
          <form @submit.prevent="handleEmailAuth" class="space-y-4">
            <div v-if="!isLoginMode" class="form-control">
              <label class="label">
                <span class="label-text">{{ $t("auth.name_label") }}</span>
              </label>
              <input
                v-model="name"
                type="text"
            ...
            <div class="form-control">
              <label class="label">
                <span class="label-text">{{ $t("auth.email_label") }}</span>
              </label>
              <input
                v-model="email"
                type="email"
            ...
            <div class="form-control">
              <label class="label">
                <span class="label-text">{{ $t("auth.password_label") }}</span>
              </label>
              <input
                v-model="password"
                type="password"
            ...
            >
              ...
              <template v-if="isSetup && !isLoginMode">
                {{ $t("auth.setup_signup_button") }}
              </template>
              <template v-else>
                {{ isLoginMode ? $t("auth.login_button") : $t("auth.signup_button") }}
              </template>
            </button>
          </form>

          <div class="divider">{{ $t("auth.or_divider") }}</div>
